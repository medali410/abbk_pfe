...
class AddTechnicianPage extends StatefulWidget {
  ...
  /// Édition : données issues de `team-directory` ou GET agents (clés `technicianId`, `companyId`, `machineIds`, etc.).
  final Map<String, dynamic>? initialData;

  const AddTechnicianPage({super.key, this.onBack, this.initialData});
...
class _AddTechnicianPageState extends State<AddTechnicianPage> {
  ...
  bool get _isEdit {
    ...
    final mid = (i['technicianId'] ?? '').toString().trim();
  ...
  String _clientRowKey(Map<String, dynamic> c) {
    final v = c['companyId'] ?? c['id'] ?? c['_id'];
  ...
  void initState() {
    ...
    if (init != null) {
      ...
      final ck = init['companyId']?.toString();
