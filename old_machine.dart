...
class _MachineDetailAiPageState extends State<MachineDetailAiPage> with TickerProviderStateMixin {
  ...
  void _applyTelemetry(Map<String, dynamic> data, {bool fromLiveStream = false}) {
    ...
    setState(() {
      ...
      if (pw != null) {
        ...
        _powerRawW = rp;
        // Le backend envoie souvent couple├ùrpm en ┬½ unit├®s grandes ┬╗ (> ~3000) : affichage kW.
  ...
  void _checkAutoAlert() {
    if (_iaProbPanne >= 60 && !_panneAlertSent) {
      ...
      final alertText = _iaProbPanne >= 75
          ? '[ALERTE CRITIQUE] Machine $_machineId : PANNE IMMINENTE (risque $_iaProbPanne%). Arr├¬t imm├®diat recommand├®.'
          : '[ALERTE CRITIQUE] Machine $_machineId : risque de panne $_iaProbPanne%. Intervention recommand├®e sous 24h.';
  ...
  Future<void> _saveIaMotorProfile() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Profil IA machine enregistr├®', style: GoogleFonts.inter()),
  ...
  /// Texte ┬½ type de panne + apr├¿s X jours ┬╗ (RUL heures ÔåÆ jours si disponible).
  String _iaHorizonJoursLine() {
    ...
    if (_iaRulEstime != null && _iaRulEstime! > 0) {
      ...
      if (r >= 48) {
        ...
        return 'Horizon mod├¿le : ~$j jour${j > 1 ? 's' : ''}';
      }
      final j = r.ceil().clamp(1, 999);
      return 'Horizon mod├¿le : ~$j jour${j > 1 ? 's' : ''}';
    }
    return 'D├®lai avant panne : non estim├® (donn├®es RUL insuffisantes)';
  }

  /// Message dÔÇÖalerte uniquement si risque ou notification serveur.
  ...
  Future<void> _emergencyStopMachine() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Voulez-vous vraiment arr├¬ter la machine $_machineId ?\n\n'
          'Cette action enverra un signal d\'arr├¬t imm├®diat au moteur. '
          'Seul un technicien sur site pourra red├®marrer la machine.',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text('CONFIRMER L\'ARR├èT', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
    ...
    try {
      await ApiService.stopMachine(
        _machineId,
        reason: 'Arr├¬t d\'urgence - Risque panne: $_iaProbPanne%',
      ...
      _socket.emit('chat_message', {
        ...
        'from': _senderRole,
        'senderName': 'Syst├¿me',
        'text': 'ÔÜá´©Å ARR├èT D\'URGENCE d├®clench├® par $_senderName ($_senderRole). '
            'Machine $_machineId arr├¬t├®e. Risque de panne: $_iaProbPanne%.',
      });
          ...
          SnackBar(
            ...
            content: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Machine $_machineId arr├¬t├®e avec succ├¿s.',
  ...
  String get _riskLabelFr {
    if (_iaProbPanne >= 70) return 'RISQUE ├ëLEV├ë';
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'S/N: $_machineId ┬À STATUS: ${_machineStopped ? 'ARR├èT' : (_iaProbPanne >= 70 ? 'INSTABLE' : 'OP├ëRATIONNEL')}',
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: '├ëcrire un message au client/technicien/conception...',
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              _machineStopped ? 'Arr├¬t' : (_lastMqttPacketAt != null ? 'Flux MQTT' : 'Veille'),
  ...
  Future<void> _loadTechniciansForMachine() async {
    ...
    try {
      ...
      // 1. Techniciens terrain (R├®pertoire ├®quipe) - Afficher TOUT
      try {
        ...
        for (final member in team) {
          ...
          // On garde tout le monde du r├®pertoire, on filtrera par machineId dans la vue
                            ...
                            child: Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text('MOTEUR ARR├èT├ë', style: GoogleFonts.spaceGrotesk(color: const Color(0xFFFF8A80), fontSize: 14, fontWeight: FontWeight.w800)),
                                ],
                              ),
                            ),
                          ),
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        ...
                        const SizedBox(width: 8),
                        _motorInfoChip('Statut', _machineStopped ? 'ARR├èT├ë' : 'EN MARCHE', _machineStopped ? _red : _green),
                      ],
                    ),
                  ),
                ),
  ...
  Widget _emergencyStopButton() {
      ...
      return Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text('ARR├èT├ë', style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.w700, color: _red)),
          ],
        ),
      );
    }
    ...
    return ElevatedButton.icon(
      ...
      label: Text(
        isUrgent ? 'ARR├èT D\'URGENCE' : 'ARR├èTER MACHINE',
      ...
      child: Column(
        ...
        children: [
                ...
                Expanded(
                  child: Text(
                    _lastMqttPacketAt == null
                        ? 'MQTT ┬À attente flux ┬À $_machineId'
                        : 'MQTT ┬À $_mqttPacketCount paquets ┬À ${_lastMqttPacketAt!.hour.toString().padLeft(2, '0')}:${_lastMqttPacketAt!.minute.toString().padLeft(2, '0')}:${_lastMqttPacketAt!.second.toString().padLeft(2, '0')}',
          ...
          if (_lastMqttPacketAt == null) ...[
            const SizedBox(height: 8),
            Text(
              'Valeurs issues de la derni├¿re t├®l├®m├®trie API jusquÔÇÖ├á r├®ception MQTT.',
          ...
          GridView.count(
            ...
            children: [
              ...
              _sensorTile('MAGN├ëTIQUE', '${_magnetic.toStringAsFixed(2)} mT', Icons.explore_rounded, const Color(0xFF90CAF9)),
              ...
              _sensorTile('PR├ëSENCE', _presence >= 0.5 ? 'OK' : 'ÔÇö', _presence >= 0.5 ? Icons.person_rounded : Icons.person_off_rounded, _presence >= 0.5 ? _green : _muted),
            ],
          ),
        ],
      ),
    ...
    return ListView(
      ...
      children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('├ëQUIPE MACHINE', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w800, color: _text, letterSpacing: 1.0)),
          const SizedBox(width: 8),
          Container(
        ...
        if (techs.isEmpty)
          Container(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 12),
              Text('Aucun technicien/maintenance assign├®', style: GoogleFonts.inter(fontSize: 13, color: _muted, fontWeight: FontWeight.w600)),
              const SizedBox(height: 4),
              Text('Ajoutez un membre ├®quipe pour cette machine', style: GoogleFonts.spaceGrotesk(fontSize: 11, color: _muted.withOpacity(0.6))),
            ]),
          )
        else
  ...
  Widget _technicianCard(Map<String, dynamic> tech) {
    ...
    final specialty = tech['specialty']?.toString() ?? (isMaintenance ? 'Maintenance' : 'G├®n├®raliste');
    return Container(
  ...
  String _maintenanceUiStatus(String raw) {
    switch (raw.toUpperCase()) {
      ...
      case 'COMPLETED':
        return 'Termin├®e';
      default:
        return raw.isEmpty ? 'Planifi├®e' : raw;
    }
  }
            ...
            onPressed: () async {
              ...
              try {
                ...
                ScaffoldMessenger.of(ctx).showSnackBar(
                  SnackBar(content: Text('Cr├®ation impossible: $e')),
                );
              }
            },
  ...
  Future<void> _showCloseCorrectiveDialog(Map<String, dynamic> order) async {
    ...
    final closed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: _panel2,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text('Cl├┤ture corrective', style: GoogleFonts.inter(color: _text, fontWeight: FontWeight.w800)),
        content: SizedBox(
          width: 430,
          child: SingleChildScrollView(
            child: Column(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                if (!ctx.mounted) return;
                ScaffoldMessenger.of(ctx).showSnackBar(
                  SnackBar(content: Text('Cl├┤ture impossible: $e')),
                );
              }
            },
  ...
  Future<void> _openConfigureMaintenanceMan() async {
    try {
      ...
      if (ok == true && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Compte maintenance enregistr├®.')),
        );
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _maintenancePanel() {
    ...
    final inProgressCount = maintenanceList
        .where((m) => m['status'] == 'Planifi├®e' || m['status'] == 'En cours')
        .length;
      ...
      children: [
          ...
          children: [
            Expanded(
              child: DropdownButtonFormField<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value: 'PENDING', child: Text('Planifi├®e')),
                  DropdownMenuItem(value: 'IN_PROGRESS', child: Text('En cours')),
                  DropdownMenuItem(value: 'COMPLETED', child: Text('Termin├®e')),
                ],
            ...
            Expanded(
              child: DropdownButtonFormField<String>(
                ...
                style: GoogleFonts.inter(color: _text),
                items: const [
                  DropdownMenuItem(value: 'ALL', child: Text('Toutes priorit├®s')),
                  DropdownMenuItem(value: 'LOW', child: Text('LOW')),
                  DropdownMenuItem(value: 'MEDIUM', child: Text('MEDIUM')),
                  DropdownMenuItem(value: 'HIGH', child: Text('HIGH')),
                  DropdownMenuItem(value: 'CRITICAL', child: Text('CRITICAL')),
        ...
        GridView.count(
          ...
          children: [
            ...
            _maintenanceStat('Termin├®es', '${maintenanceList.where((m) => m['status'] == 'Termin├®e').length}', Icons.check_circle_rounded, _green),
          ],
        ),
        const SizedBox(height: 14),
  ...
  Widget _maintenanceCard(Map<String, dynamic> m) {
    ...
    final typeColor = m['type'] == 'Corrective' ? _red
        : m['type'] == 'Pr├®dictive' ? const Color(0xFFB39DDB)
        : _cyan;
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      decoration: BoxDecoration(
          ...
          child: Row(children: [
            Container(
              ...
              child: Icon(
                m['type'] == 'Corrective' ? Icons.build_rounded
                    : m['type'] == 'Pr├®dictive' ? Icons.psychology_rounded
                    : Icons.event_rounded,
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 4),
                Text((m['date'] ?? 'ÔÇö').toString(), style: GoogleFonts.spaceGrotesk(fontSize: 10, color: _muted.withOpacity(0.7))),
                const SizedBox(width: 12),
                Icon(Icons.label_rounded, size: 10, color: _muted.withOpacity(0.5)),
                const SizedBox(width: 4),
                Text((m['tech'] ?? 'ÔÇö').toString(), style: GoogleFonts.spaceGrotesk(fontSize: 10, color: _muted.withOpacity(0.7))),
              ]),
            ])),
            const Icon(Icons.chevron_right_rounded, size: 20, color: _panel3),
          ]),
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const SizedBox(width: 10),
          Text('D├®tail maintenance', style: GoogleFonts.inter(color: _text, fontWeight: FontWeight.w800, fontSize: 16)),
        ]),
        content: SizedBox(
          width: 420,
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _detailRow('Date', (m['date'] ?? 'ÔÇö').toString(), _text),
            _detailRow('Description', (m['desc'] ?? '').toString(), _text),
            _detailRow('Source', (m['tech'] ?? '').toString(), _muted),
            _detailRow('Priorit├®', (m['priority'] ?? 'MEDIUM').toString(), _muted),
            const SizedBox(height: 16),
            if (statusRaw == 'PENDING')
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text('D├®marrer intervention', style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
            ...
            if (statusRaw == 'IN_PROGRESS') ...[
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text('Cl├┤turer corrective', style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Text(
                            _iaPanneType.isEmpty ? 'ÔÇö' : _iaPanneType,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            'Le mod├¿le re├ºoit ce type (embedding) et la calibration RUL pour l\'affichage en heures.',
            style: GoogleFonts.inter(fontSize: 11, color: _muted, height: 1.35),
          ),
          const SizedBox(height: 6),
          Text(
            'Type actif (t├®l├®m├®trie) : $_machineIaMotorType',
          ...
          const SizedBox(height: 10),
          Text(
            'Heures par unit├® RUL mod├¿le (vide = pas de conversion)',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'MACHINE ARR├èT├ëE EN TOUTE S├ëCURIT├ë',
                  style: GoogleFonts.spaceGrotesk(
                    color: Colors.white,
                    fontSize: 11,
                    fontWeight: FontWeight.w700,
                ...
                const SizedBox(height: 2),
                Text(
                  'Un technicien sur site doit red├®marrer la machine manuellement.',
                  style: GoogleFonts.inter(
  ...
  Widget _maintenanceTeamPanel() {
    ...
    final techs = (_techniciansForMachine ?? []).where((t) {
      ...
      // On filtre par ID de machine assign├®
      final List? assigned = t['machineIds'] is List ? t['machineIds'] as List : null;
      if (assigned == null) return false;
  ...
  Widget _maintenanceTechCard(Map<String, dynamic> tech) {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          isAgent ? '${tech['email'] ?? ''} ÔÇó MAINTENANCE LEAD' : (tech['email'] ?? ''),
